...
// ignore it.
					BLEAddress advertisedAddress(param->scan_rst.bda);
					bool found = false;
					log_d("Found device: %s", advertisedAddress.toString().c_str());
					...
					if (found && !m_wantDuplicates) {  // If we found a previous entry AND we don't want duplicates, then we are done.
						...
						vTaskDelay(1);  // <--- allow to switch task in case we scan infinity and dont have new devices to report, or we are blocked here
						//get device results
						BLEAdvertisedDevice *device = (m_scanResults.m_vectorAdvertisedDevices.find(advertisedAddress.toString()))->second;
						if (device->getTimesFound()<20){
							device->addValue(param->scan_rst.rssi); //store value to array
							device->setTimesFound(device->getTimesFound()+1); //count of array
						}
						//device->setTimesFound(device->getTimesFound()+1);
						device->setRSSI(device->getRSSI() + param->scan_rst.rssi); //add to RSSI for averaging later
